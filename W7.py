...
@app.route("/api/member", methods=["PATCH"])   #建立patch方式的連線
def update_member_name():
    if session.get("signIn"):   #檢查金要登入狀態
        new_name = request.json.get("name")  #提取路由/api/member的json裡的name資料
        if new_name:
            user_id = session.get("userId")  # 從金鑰中取得會員的 ID
            cursor.execute("UPDATE member SET name = %s WHERE id = %s", (new_name, user_id))
            con.commit()
            session["Name"] = new_name  # 更新金鑰的姓名
            return jsonify({"ok": True, "message": "姓名更新成功"})
        else:
